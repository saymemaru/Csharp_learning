...
namespace Gemini335
{
    internal class Gemini335Camera
    {
        ...
        /// 获取RGBD图像
        /// </summary>
        /// <param name="isOverridePic"></param>
        /// <returns></returns>
        public async Task GetRGBDImgAsync(bool isOverridePic)
        {
                ...
                while (!tokenSource.Token.IsCancellationRequested)
                {
                    ...
                    using (var frames = pipeline.WaitForFrames(100))
                    {
                        ...
                        if (colorFrame != null)
                        {
                            ...
                            SaveFile.SaveColorFrameToPng(colorFrame, "IMG", isOverridePic);
                        }
                        // 保存 depthFrame.
                        if (depthFrame != null)
                        {
                            Console.WriteLine("get depth frame");
                            SaveFile.SaveDeepFrameToTiff(depthFrame, "IMG", isOverridePic);
